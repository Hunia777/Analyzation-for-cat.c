                                                  ...
                                                  /* bufsize = Number of characters preferably read or written by each read and write call.  */
{
  ...
  while (true)
  {
    ...
    n_read = safe_read(input_desc, buf, bufsize); //input_desc는 요청한 파일의 디스크립터야. //buf에 bufsize만큼 읽어옴. //n_read에는 읽어온 byte 크기를 리턴해줌.
    if (n_read == SAFE_READ_ERROR)                //read 실패시 error처리. -1이 실패
    {
...
int main(int argc, char **argv)
{
  ...
  /*
      ...
      */
  static struct option const long_options[] = //option 구조체의 배열인 long_options가 선언됨
      {
  ...
  atexit(close_stdout); //atexit == exit인 거 같은데
  ...
  {                                                                             //return _getopt_internal (argc, (char **) argv, options, long_options, opt_index, 0, 0); it makes return the value of c. (OPTION MAKING)
    ...
    case 'T':
      ...
      /*			
        usage (EXIT_SUCCESS);	//EXIT_SUCCESS는 usage함수에서 --help 내용을 출력하도록 함.	
        break;
       */
    ...
    default:
      usage(EXIT_FAILURE); //usage에선 옵션이 예상하지 못한 값이 들어오면
                           //emit_try_help();를 call하면 거기서 ->  fprintf (stderr, _("Try '%s --help' for more information.\n"), \program_name);
    }
  }
  ...
  if (fstat(STDOUT_FILENO, &stat_buf) < 0)          //fsat()은 주어진 파일에 대한 정보를 반환한다. fstat(int fd, struct stat *buf) stat_buf는 stat type 변수
  ...
  out_dev = stat_buf.st_dev;                  //device num of output 파일을 포함하는 디바이스 ID
  out_ino = stat_buf.st_ino;                  //inode num of output inode번호
  out_isreg = S_ISREG(stat_buf.st_mode) != 0; //True if the output is a regular file.

  if (!(number || show_ends || squeeze_blank))
  {
    file_open_mode |= O_BINARY; //|= OR 연산자, 즉 file_open_mode와 O_BINARY를 or연산하여 file_open_mode를 저장.
  ...
  do
  {
    ...
    if (STREQ(infile, "-")) //(strcmp (infile, "-") == 0) 같으면 0리턴, infile의 초기값 "-"임.
    {
      have_read_stdin = true;
      input_desc = STDIN_FILENO; //옵션일 경우 input_desc는 0이 된다. (Standard In)
      if (file_open_mode & O_BINARY)
        xset_binary_mode(STDIN_FILENO, O_BINARY);
    }
    else //옵션이 아니라 파일명이 들어오면
    {
      input_desc = open(infile, file_open_mode); //input_desc는 파일이름과 파일디스크립터와 맵핑되어 해당하는 파일의 디스크립터를 open이 리턴해준다. 0보다 작으면 실패
      if (input_desc < 0)
      {
        error(0, errno, "%s", quotef(infile));
        ok = false;
    ...
    // Condition Check를 다 한다음에 이상하면 contin으로 이동 그 외에는 옵션이 없을 경우 simple_cat, 옵션이 있을경우 cat을 실행.
